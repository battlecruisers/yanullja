...
public interface MemberCouponRepository extends JpaRepository<MemberCoupon, Long> {
    // 회원의 쿠폰 사용내역 조회
    @Query(value = "SELECT mc FROM MemberCoupon mc JOIN FETCH mc.coupon c WHERE mc.isUsed = :isUsed " +
            "and mc.member.id = :memberId")
    List<MemberCoupon> findByMemberIdAndIsUsed(@Param("memberId") Long memberId, @Param("isUsed") boolean isUsed);
    ...
    @Query(value = "SELECT mc FROM MemberCoupon mc JOIN FETCH mc.coupon c WHERE c.room.id = :roomId " +
            ...
            "and mc.member.id = :memberId")
    List<MemberCoupon> findByRoomIdAndMemberId(@Param("roomId") Long roomId, @Param("memberId") Long memberId);
